...
export const networkProviders: { [network in Network]?: JsonRpcProvider } = {};
 ...
 */
export let web3Provider: JsonRpcProvider = (null as unknown) as JsonRpcProvider;
...
export const web3SetHttpProvider = async (network: Network | string) => {
  if (!isNetworkEnum(network)) {
    ...
    networkProviders[Network.mainnet] = web3Provider;
  } else {
...
export const getProviderForNetwork = async (
  ...
  if (isNetworkEnum(network) && networkProviders[network]) {
    return networkProviders[network] as JsonRpcProvider;
  }
  if (!isNetworkEnum(network)) {
    return new JsonRpcProvider(network, Network.mainnet);
  } else {
 ...
 * provider is used.
 * @returns The response from the `JsonRpcProvider.send` call.
 */
